...
class _PdfGenerationScreenState extends State<PdfGenerationScreen> {
              ...
              child: GridView.count(
                ...
                children: [
                  _buildPdfOptionCard(
                    ...
                    subtitle: 'Reporte oficial para la alcaldía',
                  ...
                  _buildPdfOptionCard(
                    title: '🏆 Por Categoría',
                    subtitle: 'Análisis por etapa del certamen',
                    icon: Icons.category,
                  ...
                  _buildPdfOptionCard(
                    title: '👨‍⚖️ Por Juez',
                    subtitle: 'Evaluaciones individuales',
                    icon: Icons.gavel,
                    color: Colors.amber.shade700,
                    onTap: () => _showJudgeSelection(),
                  ),
                  _buildPdfOptionCard(
                    title: '📊 Matriz Completa',
                    subtitle: 'Tabla Excel de todos los datos',
                    icon: Icons.grid_on,
                    color: Colors.teal.shade700,
                    onTap: () => _generateMatrixReport(),
                  ),
                  _buildPdfOptionCard(
                    title: '🥇 Ranking Final',
                    subtitle: 'Resultados para prensa',
                  ...
                  _buildPdfOptionCard(
                    title: '⚖️ Comparativo',
                    subtitle: 'Comparar dos participantes',
                    icon: Icons.compare,
                    color: Colors.pink.shade700,
                    onTap: () => _showComparisonSelection(),
                  ),
                  _buildPdfOptionCard(
                    title: '⏱️ Snapshot',
                    subtitle: 'Corte en tiempo real',
                    icon: Icons.camera_alt,
                    color: Colors.cyan.shade700,
                    onTap: () => _generateSnapshotReport(),
                  ),
                  _buildPdfOptionCard(
                    title: '📚 Libro del Certamen',
                    subtitle: 'Documento completo oficial',
                    icon: Icons.menu_book,
                    color: Colors.brown.shade700,
                    onTap: () => _generateCompleteBook(),
                  ),
                  _buildPdfOptionCard(
                    title: '🔄 Todos los Reportes',
                    subtitle: 'Generar todos los PDFs',
                    icon: Icons.all_inclusive,
                    color: Colors.red.shade700,
                    onTap: () => _generateAllReports(),
                  ),
                ],
              ),
            ),
  ...
  Future<void> _generateGeneralReport() async {
    ...
    try {
      ...
      final participantesConPromedio = await _calculateAllAverages(preguntasPorEtapa);
      ...
      for (var stage in stagesSnapshot.docs) {
        ...
        pageCounter++;
        _updateProgress('Generando página $pageCounter...', 0.8 + (0.2 * (pageCounter - 6) / stagesSnapshot.docs.length));
      }
  ...
  Future<void> _generateParticipantReport(String participantId) async {
    ...
    try {
      ...
      // Calcular estadísticas por etapa y por criterio
      ...
      final comentarios = calificacionesSnapshot.docs
          .where((doc) => doc['comentario'] != null && doc['comentario'].toString().isNotEmpty)
          .toList();
  ...
  Future<void> _generateMatrixReport() async {
    _showSuccessDialog('✅ Función en desarrollo');
  }
  ...
  Future<void> _generateSnapshotReport() async {
    _showSuccessDialog('✅ Función en desarrollo');
  }

  Future<void> _generateCompleteBook() async {
    _showSuccessDialog('✅ Función en desarrollo');
  }
  ...
  Future<List<Map<String, dynamic>>> _calculateAllAverages(
    ...
    for (var doc in participantesSnapshot.docs) {
      ...
      final participanteData = doc.data();
  ...
  Future<List<Map<String, dynamic>>> _calculateParticipantStats(
    ...
    for (var stage in stages) {
      ...
      for (var calificacion in calificacionesEtapa) {
        final data = calificacion.data();
        final puntaje = data?['puntaje'];
        final criterioId = data?['criterioId'];
      ...
      child: pw.Column(
        ...
        children: [
          ...
          pw.Container(
            ...
            decoration: pw.BoxDecoration(
              color: PdfColor.fromInt(0x1AFFFFFF), // 10% opacidad
            ...
            child: pw.Column(
              children: [
                ...
                pw.Text(
                  ...
                  style: pw.TextStyle(
                    fontSize: 14,
                    color: PdfColor.fromInt(0xCCFFFFFF), // 80% opacidad
                  ),
                ),
              ],
            ),
          ...
          pw.Text(
            ...
            style: pw.TextStyle(
              fontSize: 12,
              color: PdfColor.fromInt(0x99FFFFFF), // 60% opacidad
            ),
          ),
        ],
      ),
    ...
    final data = [
      ...
      for (var i = 0; i < participantes.length; i++)
        [
          ...
          participantes[i]['nombre'] ?? 'Sin nombre',
          participantes[i]['activo'] == true ? 'ACTIVO' : 'INACTIVO',
        ],
    ];
    ...
    final data = [
      ...
      for (var i = 0; i < jueces.length; i++)
        [
          '${i + 1}',
          jueces[i]['idInterno'] ?? '',
          jueces[i]['nombre'] ?? 'Sin nombre',
          jueces[i]['rol']?.toString().toUpperCase() ?? 'JUEZ',
          jueces[i]['activo'] == true ? 'ACTIVO' : 'INACTIVO',
        ],
    ];
          ...
          pw.Container(
            ...
            child: pw.Column(
              ...
              children: [
                pw.Row(
                  children: [
                    ...
                    pw.Container(
                      ...
                      decoration: pw.BoxDecoration(
                        color: _getStageColor(stage['color'] ?? 'blue'),
                        borderRadius: pw.BorderRadius.circular(4),
                      ),
                      child: pw.Text(
                        (stage['type']?.toString().toUpperCase() ?? ''),
                        style: pw.TextStyle(
                          fontSize: 10,
                          color: PdfColors.white,
                        ),
                ...
                pw.SizedBox(height: 5),
                pw.Text(
                  'Subtítulo: ${stage['subtitle'] ?? 'No disponible'}',
                ...
                pw.Row(
                  children: [
                    pw.Text(
                      'Puntaje máximo: ${stage['maxScore'] ?? 10} puntos',
                      style: const pw.TextStyle(fontSize: 11),
                    ),
                    pw.Spacer(),
                    pw.Text(
              ...
              pw.TableHelper.fromTextArray(
                data: [
                  ...
                  for (var calificacion in calificaciones.take(20)) // Limitar a 20 para no saturar
                    [
                      calificacion['juezId'] ?? 'N/A',
                      _getPreguntaNombre(calificacion['criterioId'] ?? '', preguntas),
                      '${calificacion['puntaje']?.toString() ?? '0'}',
                      _formatPdfDate(calificacion['fecha']),
                    ],
                ],
                headerStyle: pw.TextStyle(
                  fontWeight: pw.FontWeight.bold,
          ...
          pw.Container(
            ...
            child: pw.Column(
              ...
              children: [
                ...
                pw.SizedBox(height: 6),
                pw.Text(
                  'Fecha: ${_formatPdfDate(comentario['fecha'])}',
                  style: pw.TextStyle(
                    fontSize: 9,
                    color: PdfColors.grey600,
                  ),
